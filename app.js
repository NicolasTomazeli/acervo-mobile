...
async function renderArtworks(){
 ...
 if(q)a=a.filter(o=>[o.nome,o.patrimonio,o.artista,o.tecnica,o.localizacao].join(" ").toLowerCase().includes(q));
 ...
 a.forEach(o=>{
   ...
   const m=document.createElement("div");m.className="art-main";m.innerHTML=`<b>${esc(o.nome||"Sem título")}</b><div class=meta>${esc(o.patrimonio||"Sem patrimônio")} · ${esc(o.artista||"-")}${o.tecnica?" · "+esc(o.tecnica):""}</div><div class=meta>${esc(o.localizacao||"-")} · ${o.fotos?.length||0} foto(s)</div>`;
...
artworkForm.onsubmit=async e=>{
 ...
 const wasEditing=!!editingArtworkId;const obj={...(editingArtworkId?old:{}),patrimonio:p,nome:n,artista:(f.get("artista")||"").trim(),tecnica:(f.get("tecnica")||"").trim(),localizacao:(f.get("localizacao")||"").trim(),descricao:(f.get("descricao")||"").trim(),fotos:[...keptFotos,...referenceFiles],descriptors:[...keptDesc,...newDesc],atualizadoEm:new Date().toISOString()};
...
async function exportCsv(confId=null){
 ...
 const header=["Conferencia","Data da conferencia","Patrimonio","Nome da obra","Artista","Tecnica","Localizacao","Descricao","Qtd fotos referencia","Fotos referencia","Encontrada no periodo","Leitura","Similaridade"];
 const rows=[header.map(csvCell).join(";")];
 for(const o of works){const f=foundBy.get(o.id),fotoTxt=(o.fotos||[]).map((_,i)=>`Foto ${i+1}`).join(" | ");rows.push([c.id,new Date(c.criadoEm).toLocaleString("pt-BR"),o.patrimonio,o.nome,o.artista||"",o.tecnica||"",o.localizacao||"",o.descricao||"",o.fotos?.length||0,fotoTxt,f?"SIM":"NÃO",f?.leitura||"",f?.score!=null?(f.score*100).toFixed(1)+"%":""].map(csvCell).join(";"))}
...
async function importCatalog(file){
 ...
 for(let i=0;i<data.obras.length;i++){
   ...
   photos+=fotos.length;
   await requestP(store("obras","readwrite").add({patrimonio:pat,nome:x.nome||"Sem título",artista:x.artista||"",tecnica:x.tecnica||"",localizacao:x.localizacao||"",descricao:x.descricao||"",fotos,descriptors,sourceKey,linhaExcel:x.linha_excel||null,criadoEm:new Date().toISOString()}));
