...
async function renderHistory(){
 const cs=(await all("conferencias")).sort((a,b)=>b.id-a.id);historyList.innerHTML="";emptyHistory.classList.toggle("hidden",cs.length>0);
 let totalPending=0,totalLocated=0,totalReview=0;
 cs.forEach(c=>{
  const l=c.items.filter(i=>i.status==="localizado").length,p=c.items.length-l,review=c.items.filter(i=>i.status==="localizado"&&(i.avisoProximidade||i.avisoQualidade)).length;
  totalLocated+=l;totalPending+=p;totalReview+=review;
  const card=document.createElement("div");card.className="history-card";card.innerHTML=`<div class=history-main><div class=history-top><b>Conferência #${c.id}</b><span class="status ${p?"pending":"ok"}">${p} pendente(s)</span></div><div class=meta>${new Date(c.criadoEm).toLocaleString("pt-BR")} · ${l} localizada(s)${review?` · ${review} para revisar`:""}</div></div><div class=mini-actions><button class=mini-delete data-del="${c.id}">Excluir</button></div>`;
  card.querySelector(".history-main").onclick=()=>openResult(c.id);card.querySelector("[data-del]").onclick=e=>{e.stopPropagation();task(()=>deleteConference(c.id))};historyList.append(card);
 });
 if(resultsStats)resultsStats.innerHTML=`<div class=stat><b>${totalPending}</b><span>pendentes (todos os períodos)</span></div><div class=stat><b>${totalReview}</b><span>confirmadas, revisar recomendado</span></div>`;
}
...
async function confirmCandidate(cid,idx,oid){
 ...
 const item=c.items[idx];item.status='localizado';item.obraId=oid;item.metodo='manual';item.score=item.candidatos?.find(x=>x.obraId===oid)?.score??null;item.observacao='Confirmado manualmente.';item.avisoProximidade=false;
...
async function openResult(id){
 ...
 c.items.forEach((it,idx)=>{
  ...
  if(it.status==='localizado'&&(it.avisoProximidade||it.avisoQualidade))h+='<span class="status warn">Revisar recomendado</span>';
  ...
  h+=`<p>${esc(it.observacao||'')}</p>`;if(it.avisoQualidade)h+='<p>Foto com pouco contraste ou nitidez — considere refotografar para confirmar com mais segurança.</p>';if(it.recorte)h+='<p>Mostrando a área analisada. Foto original preservada no backup.</p>';
