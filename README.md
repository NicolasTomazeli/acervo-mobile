...
## 0.7.4 — aproveitamento dos sinais de revisão

A 0.7.3 já calculava, em `matcher.js`, dois sinais que não chegavam à interface: se a segunda candidata ficou muito próxima da nota da vencedora (`closeMatch`) e se a foto da conferência tinha pouco contraste/nitidez (`lowQuality`). Nenhum dos dois altera a regra de decisão (o limiar de 94/100 e os pesos continuam exatamente os mesmos, validados nas 2.345 consultas da 0.7.3); eles só passam a aparecer para quem está revisando:

- Confirmações automáticas com um desses sinais ganham a etiqueta **"Revisar recomendado"** na tela de resultado, além do texto explicativo já existente.
- Quando a foto está com baixo contraste/nitidez, aparece um aviso sugerindo refotografar, mesmo em itens pendentes.
- A confirmação manual (quando a pessoa escolhe a obra certa na lista) zera o aviso de proximidade — a ambiguidade já foi resolvida por alguém, não faz sentido continuar sinalizando.
- A aba Resultados ganhou um resumo consolidado de pendências e de confirmações "para revisar" somando todos os períodos, para não precisar abrir conferência por conferência atrás do que falta.

Nenhuma mudança em `matcher.js`, nos pesos, no limiar de 94/100 ou no formato do CSV exportado. O número de versão do cache do Service Worker foi atualizado para `acervo-v0.7.4` — necessário para que aparelhos que já instalaram o app anterior baixem os arquivos novos.
