...
(function (root) {
  ...
  const POLICY_VERSION='0.7.3-score94';
  const POLICY=Object.freeze({score:.94,closeMatchWarningMargin:.035});
  ...
  function decide(candidates,quality){
    ...
    // Requested rule: raw similarity >= 94/100 is sufficient. Warnings do not veto it.
    const automatic=!!top&&Number.isFinite(top.score)&&top.score>=POLICY.score;
    const closeMatch=!!candidates[1]&&margin<POLICY.closeMatchWarningMargin;
    const decisionCode=!top?'empty':automatic?'accepted':'below_threshold';
    const reason=!top?'Não há referências disponíveis para comparar.':automatic?
      `Correspondência automática com ${top.nome||'a melhor candidata'}: semelhança visual de pelo menos 94/100.`+
       (closeMatch?' Outra obra tem nota muito próxima; use “Revisar / desfazer” se a identificação estiver incorreta.':''):
      'Semelhança visual abaixo de 94/100. Compare as referências ou ajuste a área da obra.';
    return {automatic,margin,lowQuality,closeMatch,reason,decisionCode,policyVersion:POLICY_VERSION};
  }
